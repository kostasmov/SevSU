...
class DeepQLearningTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        grades.addMessage('Тестирование DQN сети...')
        ...
        if winRate < self.winThresh:
            grades.addMessage('НЕУДАЧА:\nЧастотаПобед = {} < {} порог положительной оценки'.format(winRate, self.winThresh))
            return False
        elif winRate < self.winThreshEC:
            grades.addMessage('УДАЧА:\nЧастотаПобед = {} >= {} порог положительной оценки'.format(winRate, self.winThresh))
            grades.assignFullCredit()
            return True
        else:
            grades.addMessage('УДАЧА:\nЧастотаПобед = {} >= {} порог для доп. балла'.format(winRate, self.winThreshEC))
            grades.assignFullCredit()
            grades.addPoints(1)
            return True
