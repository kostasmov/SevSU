...
% =============== Îæèäàíèå ââîäà ëèòåðû ===============
waitExit :-
    write('...íàæìèòå ëþáóþ êëàâèøó, ÷òîáû ïðîäîëæèòü...'), nl,
    get0(_), get0(_).
...
proc(1) :-
    ...
    write('Ââåäèòå îöåíêè â ôîðìàòå [Îöåíêà1, Îöåíêà2, Îöåíêà3]): '), nl, read(Îöåíêè), nl,

    assertz(ñòóäåíò(N, ÔÈÎ, Ãðóïïà, Îöåíêè)),   % äîáàâëåíèå ôàêòà â ÁÄ
    write('Ñòóäåíò '), display(ÔÈÎ), write(' áûë äîáàâëåí â ÁÄ'), nl, nl,
    waitExit.
...
proc(2) :-
    ...
    read(N), nl,    % ââîä íîìåðà ñîòðóäíèêà

    retract(ñòóäåíò(N, _, _, _)),   % óäàëåíèå çàïèñè î ñòóäåíòå

    % çàïèñü óäàëîñü íàéòè óäàëèòü
    write('Ñòóäåíò '), display(N), write(' áûë óñïåøíî óäàëåí èç ÁÄ'), nl, nl,
    waitExit,

    ! ; % çàâåðøåíèå èëè àëüòåðíàòâà

    % çàïèñü íå íàéäåíà - óäàëèòü íå÷åãî
    write('Òàêîãî ñòóäåíòà â áàçå äàííûõ íåò'), nl, nl,
    waitExit.


% ================== 3. Ïðîñìîòð ÁÄ ==================
proc(3) :-
    % èçâëåêàåì çàïèñè èç ÁÄ ïîêà îíè íå çàêîí÷àòñÿ
    ñòóäåíò(N, ÔÈÎ, Ãðóïïà, Îöåíêè),    

    write('Íîìåð: '), display(N), nl,
    write('ÔÈÎ: '), display(ÔÈÎ), nl,
    write('Ãîä ïîñòóïëåíèÿ: '), display(Ãðóïïà), nl,
    write('Îöåíêè: '), write(Îöåíêè), nl, nl,

    fail ;      % åñëè çàïèñü íàøëàñü - èñêóñòâåííî âåðíèñü (false) è ïîèùè åù¸
                % èíà÷å êîíåö ïðîñìîòðà

    waitExit.   % çàâåðøåíèå - çàïèñåé áîëüøå íåò
