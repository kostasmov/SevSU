...
proc(6) :-
    ...
    write('Отношение R2: студенты из группы ПИ1'), nl,
    подмножество_студентов('ПИ1', R2),  % R2 - список студентов группы ПИ1
...
объединение_R1_R2(N, ФИО, Группа, Оценки) :-
    ...
    студент_R(N, ФИО, Группа, Оценки), Группа='ПИ1'.
...
пересечение(R_Intersect) :-
    findall(студент_R1_2(N, ФИО, Группа, Оценки),
          пересечение_R1_R2(N, ФИО, Группа, Оценки), R_Intersect),
    % проверка что пересечение - пустое множество
    ( R_Intersect = [], write('Пересечение пусто!'), nl
    ; R_Intersect \= [] ).
%-------------------------------------------------------------------------------------------------------
...
подмножество_студентов(Группа, R) :-
    bagof(студент_R(N, ФИО, Группа, Оценки),
          студент(N, ФИО, Группа, Оценки), R).

% перенос списка R в объекты БД
список_в_бд([H|T]) :-
    H = студент_R(N, ФИО, Группа, Оценки),
...
подмножество_отличников(Отличники) :-
    findall(студент(N, ФИО, Группа, Средний),
...
вывод_отличников([]).
вывод_отличников([студент(_, ФИО, Группа, Средний)|T]) :-
